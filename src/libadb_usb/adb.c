...
/*hjw add, for root power @user release, bug[4432]. begin */
    char value[PROPERTY_VALUE_MAX];
    property_get("persist.sys.secret_root", value, "0");
    if (strcmp(value, "1")  == 0)  
    {
        return 0;
    }
/*hjw add, for root power @user release, bug[4432]. end */
    return 1;
...
//hjw add begin,for no adb auth when secret root enabled.
    property_get("persist.sys.secret_root", value, "0");
    auth_enabled = auth_enabled &&(!strcmp(value, "0"));
//hjw add end,for no adb auth when secret root enabled.
...
#if HIPAD_LOCK
#define LOCK_FILE  "/persist/lock.bin"
#define SD_KEY ("/storage/sdcard1/keys")
#define HIPAD_LOCK_NUM 1 
#define HIPAD_ALL_LOCK 0x01
#define HIPAD_SIM_LOCK 0x02
#define HIPAD_USB_LOCK 0x04
...
int hipad_read_lock_items_from_efs(char *buffer)
{
    ...
    if(fp == NULL) {
        perror("failed to open lock.bin");
        return -1;
    }
    count = fread(buffer, HIPAD_LOCK_NUM, 1, fp);
    fclose(fp);

    return 0;
}

int hipad_write_lock_items_to_efs(char *buffer)
{
    ...
    if(fp == NULL) {
        perror("failed to open lock.txt");
        return -1;
    }
    count = fwrite(buffer, HIPAD_LOCK_NUM, 1, fp);
    fclose(fp);
...
#else
    #if HIPAD_LOCK
    char lock_file[HIPAD_LOCK_NUM];
    int lock_value = 0;
	char key[6]={0xa0,0x0b,0x0f,0xcd,0xe0,0x00};
	char key_buf[6]={0};
	int sd_key_status=0;
	FILE*  fd;
	const char* devlist[] = {"/dev/block/mmcblk1p1","/dev/block/mmcblk1"};
	int i=0;
    #endif
...
if(!(access(SD_KEY, 0))){
	    fd=fopen(SD_KEY,"rb");
	    if(fd==NULL){
   		perror("can't open file /storage/sdcard1/keys!\n");
	  	}
  	  else{
		  fread(key_buf,sizeof(char),6,fd);
		  if(!strcmp(key_buf,key))
			sd_key_status=1;
		  else
		   perror("File /storage/sdcard1/keys's content illegal!\n");
		 fclose(fd);
	    }
}
else
{
  for (i = 0; i < 2; i++)  //read signature file on SD card
	 {
	  if (access(devlist[i], F_OK) == 0)
	   {
	    if(0==mount(devlist[i], "/storage/sdcard1", "vfat", 0, NULL)){
	    if(!(access(SD_KEY, 0))){
		    fd=fopen(SD_KEY,"rb");
		    if(fd==NULL){
	   		perror("can't open file /storage/sdcard1/keys!\n");
			  ...
			  fclose(fd);
	   	    ...
	   	    perror("Signature file /storage/sdcard1/keys not exist!\n");
	   umount(devlist[i]);
	   break;
	    }
	  }
    ...
    memset(lock_file, 0, HIPAD_LOCK_NUM);
    hipad_read_lock_items_from_efs(lock_file);
    lock_value = atoi(lock_file);

   if(0==sd_key_status&&(!(lock_value & HIPAD_ALL_LOCK))) //Lock All
    {
        //adb lock
        property_set("persist.sys.adb_lock","1");
        //diag lock
        property_set("persist.sys.diagnotic_lock","1");
        property_set("persist.sys.usb.config","serial_tty,mass_storage,adb");
        //sim lock
        property_set("persist.sys.lock.evdo","1");
        property_set("persist.sys.lock.ruim","1");

        itoa(1,lock_file,HIPAD_LOCK_NUM,10);
        hipad_write_lock_items_to_efs(lock_file);
    }
	
   if(1==sd_key_status&&(lock_value & HIPAD_ALL_LOCK)) //Unlock All
	{
		//adb unlock
        property_set("persist.sys.adb_lock","0");
        //diag unlock
        property_set("persist.sys.diagnotic_lock","0");
        property_set("persist.sys.usb.config","diag,serial_smd,serial_tty,rmnet_bam,mass_storage");
        //sim unlock
        property_set("persist.sys.lock.evdo","0");
        property_set("persist.sys.lock.ruim","0");

        itoa(0,lock_file,HIPAD_LOCK_NUM,10);
        hipad_write_lock_items_to_efs(lock_file);
	}
 #endif  
